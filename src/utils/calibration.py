...
class UnifiedCalibrationTool:
    ...
    def run_coordinate_capture(config, preserve_sessions=False):
        """Run the coordinate capture tool as a standalone function."""
        try:
            # Create a simple root window
            root = tk.Tk()
            root.title("Claude UI Coordinate Capture")
            root.geometry("400x200")
            
            # Create a label with instructions
            ttk.Label(
                root, 
                text="This tool helps you capture mouse coordinates for UI elements.\n\n"
                    "Click 'Capture' to start, then position your mouse over the\n"
                    "element you want to click and wait for 3 seconds.",
                justify=tk.CENTER
            ).pack(pady=20)
            
            # Variable to store selected element name
            ...
            def capture_coords():
                ...
                ui_elements[element_name]["click_coordinates"] = [x, y]
                ui_elements[element_name]["use_coordinates_first"] = True
                
                # Save configuration with session preservation if requested
                ...
                # Show result
                root.deiconify()
                messagebox.showinfo("Success", f"Captured coordinates ({x}, {y}) for element '{element_name}'")
            
            # Create a dropdown to select element
            ttk.Label(root, text="Select element:").pack(pady=(0, 5))
            element_dropdown = ttk.Combobox(root, textvariable=element_var)
            element_dropdown.pack(pady=(0, 10))
            
            # Fill dropdown with element names from config
            ui_elements = config.get("ui_elements", {})
            element_dropdown['values'] = list(ui_elements.keys())
            
            # Add option to add new element
            def add_new_element():
                name = simpledialog.askstring("New Element", "Enter name for new element:")
                if name and name.strip():
                    # Add to dropdown
                    values = list(element_dropdown['values'])
                    values.append(name)
                    element_dropdown['values'] = values
                    element_dropdown.set(name)
            
            # Create buttons
            button_frame = ttk.Frame(root)
            button_frame.pack(fill=tk.X, pady=10)
            
            ttk.Button(button_frame, text="New Element", command=add_new_element).pack(side=tk.LEFT, padx=10)
            ttk.Button(button_frame, text="Capture", command=capture_coords).pack(side=tk.LEFT, padx=10)
            ttk.Button(button_frame, text="Close", command=root.destroy).pack(side=tk.RIGHT, padx=10)
            ...
            logging.error(f"Error in coordinate capture: {e}")
            return False
